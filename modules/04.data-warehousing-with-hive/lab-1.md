...
1. Using the HDFS CLI, take a look at the data used for this lab at `/education/dsti_2022_spring/resources/lab4/nyc_drivers/drivers.csv`

2. Copy the `nyc_drivers` folder to your user directory in HDFS:

   ```bash
   hdfs dfs -mkdir -p "/education/dsti_2022_spring/$USER/lab4"
   
   hdfs dfs -cp /education/dsti_2022_spring/resources/lab4/nyc_drivers "/education/dsti_2022_spring/$USER/lab4"
   ```
...
2. Check that your table was created using the HDFS CLI at `/warehouse/tablespace/managed/hive/dsti_2022_spring.db/${hiveUsername}_nyc_drivers` (should be empty)
...
3. Check what the data looks like in the managed table using the HDFS CLI at `/warehouse/tablespace/managed/hive/dsti_2022_spring.db/${username}_nyc_drivers`
